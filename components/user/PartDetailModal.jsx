...
import { addItemToCart, updateCartItemQty, removeItemFromCart } from '../../store/slices/buyerSlice';
import { X, Wrench, ShoppingBag, Truck, ShieldCheck, Check, Minus, Plus, Trash2, AlertTriangle } from 'lucide-react';
...
const PartDetailModal = ({ part, onClose }) => {
  ...
  const { cart } = useSelector((state) => state.buyer);
  const inCart = cart.find(item => item.productId === part.id);

  // Use stock from prop if available (mock/real data structure check)
  ...
  const handleUpdateQuantity = (newQty) => {
    if (inCart) {
      ...
      dispatch(updateCartItemQty({ id: inCart.id, quantity: newQty }));
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Left: Image Placeholder - Keep dark for contrast */}
        <div className="w-full md:w-2/5 bg-neutral-900 relative min-h-[200px] md:min-h-full p-8 flex flex-col items-center justify-center border-r border-nothing-gray overflow-hidden">
          {part.images && part.images.length > 0 ? (
            <img src={part.images[0].url} alt={part.title} className="w-full h-full object-cover" />
          ) : (
        ...
        <div className="w-full md:w-3/5 flex flex-col bg-nothing-dark p-8">
          <div className="flex-1 space-y-6">
            <div>
              ...
              </div>              <h2 className="text-2xl font-medium tracking-tight mt-3 text-nothing-white">{part.title}</h2>
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-nothing-muted">Verified by Team Apex.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Truck className="text-nothing-white shrink-0" size={20} />
                <div>
                  <h4 className="text-sm font-medium text-nothing-white">Fast Delivery</h4>
                  <p className="text-xs text-nothing-muted">Delivery by tomorrow if ordered now.</p>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-nothing-gray pt-4">
              <p className="text-sm text-nothing-muted leading-relaxed">
                High-performance replacement part suitable for most standard models.
                Ensure compatibility with your vehicle year before purchase.
              </p>
            </div>
          </div>
