...
import { setSelectedMechanicOrder } from '../../store/slices/mechanicSlice';
...
const MechanicOrderModal = ({ onCancelClick }) => {
    const dispatch = useDispatch();
    const { selectedOrder } = useSelector((state) => state.mechanic);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
                ...
                onClick={() => dispatch(setSelectedMechanicOrder(null))}
            />
                            ...
                            <div key={item.id} className="flex gap-4 p-4 bg-nothing-black/30 rounded-2xl border border-nothing-gray">
                                <div className="w-20 h-20 bg-nothing-white/5 rounded-xl border border-nothing-gray overflow-hidden flex items-center justify-center shrink-0">
                                    {item.product.images && item.product.images.length > 0 ? (
                                        ...
                                        <Wrench size={24} className="text-nothing-muted" />
                                    )}
                                </div>
...
export default MechanicOrderModal;